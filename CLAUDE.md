...
2. **Stay in your lane.** Only create/edit files inside the directories your TASK.md assigns to you. Never edit: `src/App.tsx`, `src/main.tsx`, `src/store/*`, `src/lib/*`, `src/styles/*`, `package.json`, vite config, CI. If you believe a shared file must change, write the request into your handoff file and work around it locally.
...
9. When done, write a short summary (what you built, decisions, anything left) into `HANDOFF-<feature>.md` at repo root (e.g. `HANDOFF-history.md` — unique per feature so merges never conflict) and make a final commit.
